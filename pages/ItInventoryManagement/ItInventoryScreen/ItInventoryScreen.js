...
import ItInventoryCard from "../../../component/inventoryCard/ItInventoryCard";
...
export const locationData = [
  {
    ...
    subLocations: [
      { key: "Lab 1", label: "Lab 1", icon: "business-outline" },
      { key: "Lab 2", label: "Lab 2", icon: "business-outline" },
      { key: "Lab 3", label: "Lab 3", icon: "business-outline" },
      { key: "Counseling Cabin", label: "Counseling Cabin", icon: "people-outline" },
  ...
  },
  {
    ...
    subLocations: [
      ...
      { key: "Lab 2", label: "Lab 2", icon: "business-outline" },
      { key: "Lab 3", label: "Lab 3", icon: "business-outline" },
      { key: "Lab 4", label: "Lab 4", icon: "business-outline" },
      { key: "Theory", label: "Theory", icon: "school-outline" },
      { key: "Ground Floor", label: "Ground Floor", icon: "home-outline" },
    ],
  },
];

export default function ItInventoryScreen({ navigation }) {
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      await apiGet(
        "/get-it-inventory/",
        { mainLocation: selectedMain.key, location: selectedSub.key },
        setInventories
      );
    } catch (err) {
    ...
    <View style={{ flex: 1, backgroundColor: "#f5f7fa" }}>
      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity
          ...
          onPress={() => openOptions("mainLocation")}
        >
          <Ionicons name="location-outline" size={22} color="darkblue" />
          <Text style={styles.inputText}>
            {selectedMain?.label || "Select Main Location"}
          </Text>
        ...
        {selectedMain && (
          <TouchableOpacity
            style={[styles.inputRow, { marginTop: 16 }]}
            onPress={() => openOptions("location", selectedMain)}
          >
            <Ionicons name="business-outline" size={22} color="darkblue" />
            <Text style={styles.inputText}>
              {selectedSub?.label || "Select Sub Location"}
            </Text>
            <Ionicons name="chevron-down" size={20} color="#666" />
          </TouchableOpacity>
        )}
        <TouchableOpacity style={styles.submitBtn} onPress={handleFilter}>
          <Text style={styles.submitText}>
            {loading ? <ActivityIndicator size={19} color="white" /> : "Filter"}
          </Text>
        </TouchableOpacity>
      </View>
      {/* Inventory List */}
      <FlatList
        ...
        renderItem={({ item }) => <ItInventoryCard navigation={navigation} inventory={item} />}
        ListEmptyComponent={
          !loading && (
            <Text style={{ margin: 20, color: "#999" }}>No inventories found.</Text>
          )
        }
